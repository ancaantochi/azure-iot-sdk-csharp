...
namespace Microsoft.Azure.Devices.Client.Test.HsmAuthentication
{
    ...
    public class SasTokenBuilderTest
    {
        ...
        public void TestBuildSasToken_ShouldReturnSasToken()
        {
            ...
            string expiry = double.Parse(secondsFromBaseTime.TotalSeconds.ToString(CultureInfo.InvariantCulture)).ToString();
            ...
            Assert.AreEqual(expiry, token.ExpiresOn.Subtract(SharedAccessSignatureConstants.EpochTime).TotalSeconds.ToString());
        }
    }
}
